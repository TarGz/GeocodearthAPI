...
class FlickrSet  extends proxy 
{
	...
	{	
		...
		return $this->generateGeoRSS();
	}
	
	private function getPhotos( $setid, $count )
	{
		$set = $this->executeRequest( "flickr.photosets.getPhotos", array( array( "name" => "photoset_id", "value" => $setid ), array( "name" => "per_page", "value" => $count ) ) );
		
		// Récupération du photoset
		$this->photoset = $set[ "rsp" ][ "photoset" ];
		...
		private function getImages()
    {
        foreach( $this->photos as $id=>$value )
        {
            ...
            if ( $images[ "stat" ] == "ok" )
            {
				...
				$this->photos[ $id ][ "image" ] = $images[ $n-1 ];
            }
        }
    }
	...
	private function generateGeoRSS()
	{
		...
		//echo $str1;
		$s=$str1."<channel>\n";
	 	$s=$s."<title>Twitter public timeline in GeoRss</title>\n";
	    $s=$s."<link>http://www.geocodearth.com/api/proxy/twitter_proxy.php</link>\n";
	    $s=$s."<description>See what's going on in the Twiter Universe</description>\n";
	...
	private function formatItem( $photoInfos )
    {
        ...
        $s .= "<media:content url='".$photoInfos[ "image" ][ "source" ]."' type='image/jpeg' height='".$photoInfos[ "image" ][ "height" ]."' width='".$photoInfos[ "image" ][ "width" ]."' />\n";
        $s .= "<media:title>".$photoInfos[ "title" ]."</media:title>\n";
        $s .= "<media:thumbnail url='".$photoInfos[ "thumb" ][ "source" ]."' type='image/jpeg' height='".$photoInfos[ "thumb" ][ "height" ]."' width='".$photoInfos[ "thumb" ][ "width" ]."'  />\n";
        $s .= "</item>\n";
