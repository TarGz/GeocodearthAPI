...
abstract class Phirehose
{
  ...
  protected function connect() 
  {
    ...
    do {
      ...
      if (!$this->conn) {
		...
		$msg = 'Failed to connect to stream: ' . $errStr . ' (' . $errNo . '). Sleeping for ' . $tcpRetry . ' seconds.';
        $this->log('Phirehose: '.$msg);
        sleep($tcpRetry);
        continue;
      }
      ...
      if ($httpCode != 200) {
		...
		$msg = 'Failed to connect to stream: ' . $errStr . '. Sleeping for ' . $httpRetry . ' seconds.';
        $this->log('Phirehose: '.$msg);
        sleep($httpRetry);
        continue;
  ...
  {
    @error_log($message, 0);
	echo '<status><err>1</err><errMess>'.$message.'</errMess></status>';
  }
